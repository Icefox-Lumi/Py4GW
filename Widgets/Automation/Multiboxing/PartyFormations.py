...
import Py4GW
from HeroAI.party_formations import ASSIGNMENT_ACCOUNT
from HeroAI.party_formations import ASSIGNMENT_HERO
from HeroAI.party_formations import ASSIGNMENT_UNASSIGNED
from HeroAI.party_formations import MAX_FORMATION_SPOTS
from HeroAI.party_formations import TARGET_MODE_IDENTITY
from HeroAI.party_formations import TARGET_MODE_PARTY_SLOT
from HeroAI.party_formations import FormationCooldowns
from HeroAI.party_formations import FormationAssignment
from HeroAI.party_formations import PartyFormation
from HeroAI.party_formations import LEADER_PARTY_POSITION
from HeroAI.party_formations import PREFLIGHT_STATUS_WOULD_TARGET
from HeroAI.party_formations import apply_formation
from HeroAI.party_formations import assignment_from_member
from HeroAI.party_formations import assignment_has_target
from HeroAI.party_formations import assignment_spot_label
...
MODULE_ICON = 'Textures\\Module_Icons\\Party Formations.png'
...
def _account_ini_file_exists(filename: str) -> bool:
    try:
        account_path = IniManager().get_path()
    except Exception:
        return True
    if not account_path:
        return True
    return os.path.exists(os.path.join(account_path, WINDOW_INI_PATH, filename))


def _apply_new_window_default_size(ini_key: str, default_size: tuple[int, int]) -> None:
    try:
    ...
    node.width = int(default_size[0])
    node.height = int(default_size[1])
...
def _ensure_window_ini_key(filename: str, default_size: tuple[int, int]) -> str:
    file_exists = _account_ini_file_exists(filename)
    try:
        ini_key = IniManager().ensure_key(WINDOW_INI_PATH, filename)
    except Exception:
        return ''
    if not ini_key:
        return ''
    if not file_exists:
        _apply_new_window_default_size(ini_key, default_size)
    return ini_key


def _ensure_window_ini_keys() -> None:
    global main_window_ini_key
    global canvas_editor_window_ini_key

    if not main_window_ini_key:
        main_window_ini_key = _ensure_window_ini_key(MAIN_WINDOW_INI_FILENAME, MAIN_WINDOW_DEFAULT_SIZE)
    if not canvas_editor_window_ini_key:
        canvas_editor_window_ini_key = _ensure_window_ini_key(
            CANVAS_EDITOR_WINDOW_INI_FILENAME,
            CANVAS_EDITOR_WINDOW_DEFAULT_SIZE,
        )


def _apply_new_floating_icon_default_position(ini_key: str) -> None:
    try:
        node = IniManager()._get_node(ini_key)
    except Exception:
        node = None
    ...
    node.x_pos = int(FLOATING_ICON_DEFAULT_POS[0])
    node.y_pos = int(FLOATING_ICON_DEFAULT_POS[1])
...
def _ensure_floating_ui_key() -> str:
    global floating_ui_ini_key

    if floating_ui_ini_key:
        return floating_ui_ini_key
    file_exists = _account_ini_file_exists(FLOATING_UI_INI_FILENAME)
    try:
        floating_ui_ini_key = IniManager().ensure_key(WINDOW_INI_PATH, FLOATING_UI_INI_FILENAME)
    except Exception:
        floating_ui_ini_key = ''
    if floating_ui_ini_key and not file_exists:
        _apply_new_floating_icon_default_position(floating_ui_ini_key)
    return floating_ui_ini_key
...
def _ensure_floating_ui():
    ...
    if floating_button is None:
        floating_button = ImGui.FloatingIcon(
            ...
            start_pos=FLOATING_ICON_DEFAULT_POS,
...
def _begin_persistent_window(ini_key: str, name: str, flags: int) -> bool:
    if ini_key:
        return ImGui.Begin(ini_key, name, flags=flags)
...
def _begin_persistent_window_with_close(
    ...
    if ini_key:
        return ImGui.BeginWithClose(ini_key, name, p_open=p_open, flags=flags)
...
def _valid_hotkey_key(key: Key) -> bool:
    return key not in (Key.Unmapped, Key.Unused, Key.Unmappable, Key.VK_0x00)
...
def _release_hotkey_latches() -> None:
    for formation in formations:
        ...
        try:
            if not PyImGui.is_key_down(key.value):
                latched_hotkey_ids.discard(formation.formation_id)
        except Exception:
            latched_hotkey_ids.discard(formation.formation_id)
...
def _draw_action_row_label(text: str) -> None:
    try:
        ...
        if callable(frame_height_getter):
            frame_height = float(frame_height_getter() or 0.0)
        else:
...
def _load_geometry_presets() -> tuple[list[dict], list[str]]:
    ...
    try:
        ...
        return [], [f'Preset library could not be loaded: {exc}']

    if not isinstance(raw, dict):
        return [], ['Preset library must be a JSON object.']
    if raw.get('type') != GEOMETRY_PRESET_LIBRARY_TYPE:
        return [], ['Preset library has an unsupported type.']
    version = raw.get('version')
    if isinstance(version, bool) or not isinstance(version, int) or version != GEOMETRY_PRESET_LIBRARY_VERSION:
        return [], ['Preset library has an unsupported version.']
    ...
    for index, preset_raw in enumerate(presets_raw):
        preset, error = _validate_geometry_preset(preset_raw, index)
        if error:
            errors.append(error)
            continue
...
def _write_geometry_presets(presets: list[dict]) -> tuple[bool, str]:
    ...
    try:
        write_json_file_atomic(path, payload, indent=2, ensure_ascii=True)
    except (OSError, TypeError, ValueError) as exc:
        return False, f'Could not save geometry presets: {exc}'
...
def _finish_geometry_preset_save(
    ...
    if not ok:
        _set_status(message, details=errors, message_type=Py4GW.Console.MessageType.Warning)
        return False
            ...
            if inside and canvas_dragging_assignment_index < len(formation.assignments):
                ...
                if canvas_drag_active:
                    ...
                    if use_position_draft:
                        ...
                        assignment = formation.assignments[canvas_dragging_assignment_index]
                        old_offset_x, old_offset_y = _assignment_offset_tuple(assignment)
                    if (
                        ...
                        if use_position_draft:
                            ...
                            assignment.offset_x = offset_x
                            assignment.offset_y = offset_y
...
def _draw_geometry_preset_preview(preset: dict, preview_scale: float) -> None:
    ...
    preview_height = GEOMETRY_PRESET_PREVIEW_HEIGHT
    child_flags = PyImGui.WindowFlags.NoTitleBar | PyImGui.WindowFlags.NoResize | PyImGui.WindowFlags.NoMove
    opened = PyImGui.begin_child('PartyFormationGeometryPresetPreview', (preview_width, preview_height), True, child_flags)
    if opened:
        canvas_pos = PyImGui.get_cursor_screen_pos()
        left = float(canvas_pos[0])
        top = float(canvas_pos[1])
...
def _draw_canvas_tools_settings(formation: PartyFormation) -> None:
    ...
    if _begin_canvas_tools_subsection(
        ...
        'Nudge uses the primary selected spot; mirror and align affect every spot; distribute uses the selection group.',
    ):
...
def _draw_canvas_selected_spot_details(formation: PartyFormation) -> None:
    ...
    assignment = formation.assignments[selected_index]
    draft_offsets = _canvas_position_draft_offsets_for(formation)
    using_draft = draft_offsets is not None and selected_index < len(draft_offsets)
    offset_x, offset_y = draft_offsets[selected_index] if using_draft else _assignment_offset_tuple(assignment)
    enabled = bool(getattr(assignment, 'enabled', True))
...
def _export_shape_to_clipboard(formation: PartyFormation) -> None:
    ...
    _set_status(result.status(), details=result.details, message_type=Py4GW.Console.MessageType.Info)
...
def _import_shape_from_clipboard() -> None:
    ...
    _save()
    _set_status(result.message, details=result.details, message_type=Py4GW.Console.MessageType.Info)
...
def _duplicate_target_details(formation: PartyFormation) -> list[str]:
    targets: dict[tuple[str, object], dict[str, object]] = {}
    for index, assignment in enumerate(formation.assignments):
            ...
            if not assignment_has_target(assignment):
                ...
                if capture_clicked:
                    ...
                    _set_status(
                        message,
                        log=True,
                        message_type=Py4GW.Console.MessageType.Info if ok else Py4GW.Console.MessageType.Warning,
                    )
                PyImGui.same_line(0, 3)
...
def main() -> None:
    ...
    _set_main_window_visible(window_open, persist=True, expand_on_show=False)
...
if __name__ == '__main__':
    try:
        ...
        _log(f'ImportError encountered: {exc}', Py4GW.Console.MessageType.Error)
        _log(f'Stack trace: {traceback.format_exc()}', Py4GW.Console.MessageType.Error)
    except ValueError as exc:
        _log(f'ValueError encountered: {exc}', Py4GW.Console.MessageType.Error)
        _log(f'Stack trace: {traceback.format_exc()}', Py4GW.Console.MessageType.Error)
    except TypeError as exc:
        _log(f'TypeError encountered: {exc}', Py4GW.Console.MessageType.Error)
        _log(f'Stack trace: {traceback.format_exc()}', Py4GW.Console.MessageType.Error)
    except Exception as exc:
        _log(f'Unexpected error encountered: {exc}', Py4GW.Console.MessageType.Error)
        _log(f'Stack trace: {traceback.format_exc()}', Py4GW.Console.MessageType.Error)
